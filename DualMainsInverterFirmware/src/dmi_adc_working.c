...
void dmi_adc_init(void) {
	...
	_adc_config.negative_input = ADC_NEGATIVE_INPUT_GND;
	...
	_adc_config.gain_factor    = ADC_GAIN_FACTOR_1X;
	_adc_config.accumulate_samples = ADC_ACCUMULATE_SAMPLES_16;
...
bool dmi_adc_read_conversion(uint16_t* result) {
	uint16_t adc_temp;
	if (adc_read(&_adc_inst, &adc_temp) == STATUS_OK) {
		*result = adc_temp;
		return true;
	} else {
		return false;
